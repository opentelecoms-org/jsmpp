 ...
 * It also parses the final status "Successful".
 * Use with DeliverSm.getDeliveryReceipt()
 *
 * @author pmoerenhout
 */
public class CustomDeliveryReceiptStripper implements DeliveryReceiptStrip<DeliveryReceipt> {

  private static final Logger LOG = LoggerFactory.getLogger(CustomDeliveryReceiptStripper.class);

  private static final Pattern PATTERN = Pattern
      .compile("^id:([0-9]*) sub:([0-9]{3}) dlvrd:([0-9]{3}) submit date:([0-9]{10}) done date:([0-9]{10}) stat:([a-zA-Z]*)$");
  ...
  public DeliveryReceipt strip(DeliverSm deliverSm) throws InvalidDeliveryReceiptException {
    if (MessageType.SMSC_DEL_RECEIPT.containedIn(deliverSm.getEsmClass()) || MessageType.SME_DEL_ACK.containedIn(deliverSm.getEsmClass())) {
      ...
      return getDeliveryReceipt(deliverSm.getShortMessage());
    } else {
  ...
  private DeliveryReceipt getDeliveryReceipt(byte[] shortMessage) throws InvalidDeliveryReceiptException {
